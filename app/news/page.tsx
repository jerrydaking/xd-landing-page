...
import { DEFAULT_NEWS_ARTICLES } from "@/lib/defaultNewsArticles";
...
function loadArticlesFromStorage(): Article[] {
  ...
  try {
    ...
    if (!raw) return DEFAULT_NEWS_ARTICLES;
    const normalized = normalizeArticles(JSON.parse(raw));
    if (normalized.length === 0) return DEFAULT_NEWS_ARTICLES;
...
function NewsContent() {
  ...
  const [articles, setArticles] = useState<Article[]>(DEFAULT_NEWS_ARTICLES);
